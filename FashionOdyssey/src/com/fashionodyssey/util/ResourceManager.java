...
public class ResourceManager {
    ...
    // 作物類型映射
    private static final Map<String, String> CROP_KEY_MAP = Map.of(
        "棉花", "cotton",
        "玫瑰", "rose",
        "向日葵", "sunflower",
        "鬱金香", "tulip",
    ...
    public void buySeed(String cropType) {
        ...
        if (type != null && useMoney(type.getSeedCost())) {
            addResource(seedType, 10);  // 一次購買10個種子
        }
    }
    
    public void addHarvestedCrop(String cropType) {
        String harvestedKey = HARVESTED_PREFIX + CROP_KEY_MAP.get(cropType);
        int amount = cropType.equals("棉花") ? 
            (int)(Math.random() * 3) + 3 :  // 棉花 3-5個
            (int)(Math.random() * 3) + 1;   // 其他 1-3個
        
        resources.merge(harvestedKey, amount, Integer::sum);
    ...
    public void buyFertilizer() {
        int fertilizerCost = 20;  // 修改肥料價格為20元
        if (money >= fertilizerCost) {
            money -= fertilizerCost;
            addResource("fertilizer", 1);
            EventManager.getInstance().fireEvent(
                new GameEvent("UPDATE_STATUS", "成功購買肥料")
            );
        } else {
            EventManager.getInstance().fireEvent(
    ...
    public void consumeResource(String type, int amount) {
        if (getResourceAmount(type) >= amount) {
            resources.put(type, getResourceAmount(type) - amount);
            notifyResourceChange();
        }
    }
    ...
    public void notifyInventoryChange() {
        ...
        int[] amounts = {
            getResourceAmount("fertilizer"),
            getResourceAmount("棉花_seeds"),
            getResourceAmount("玫瑰_seeds"),
            getResourceAmount("向日葵_seeds"),
            getResourceAmount("鬱金香_seeds"),
            getResourceAmount("薰衣草_seeds"),
